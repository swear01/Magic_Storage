...
SCENARIOS = {
    ...
    "crafting-fuel-page": {
        ...
        "checks": [
            ...
            "Confirm Installed Machines uses two flow rows and both Installed Machines and Energy Reserves distribute across the available panel width instead of clustering at the left, without clipping.",
            ...
            "With every currently registered machine slot empty, confirm every machine-generated total stays at zero.",
            "Shift-click the supplied stack of three Furnaces into Installed Machines; the hover tooltip must report the exact rate and only Smelting Energy may rise.",
            "Remove the Furnace stack and confirm generation stops while the accumulated Smelting Energy remains.",
            "Install the supplied Crafting Table, Stonecutter, Smithing Table, and iron axe in their exact Installed Machines slots; these four entries must not increase any machine-generated energy total.",
            "With player-inventory ingredients enabled, verify station gating dynamically: Crafting Table exposes an Oak Log crafting recipe, Stonecutter exposes a cobblestone result, Smithing Table exposes the Netherite Smithing Transform, and the installed axe exposes an Oak Log strip transformation.",
            ...
            "Inspect the Storage Core, six Storage Unit tiers, both terminals, Import/Export Bus faces, and Remote Terminal item: every new model texture must read as crisp native 16×16 pixel art with a coherent dark stone plus restrained amethyst/cyan accent, while adjacent tiers and bus faces remain distinguishable.",

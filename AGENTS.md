...
- CI lives in `.github/workflows/ci.yml` and must keep `./gradlew build`, minimum/latest-compatible EMI API compilation, `./gradlew runGameTestServer`, `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover scripts`, and the `./gradlew runData` datagen drift check green. TerraformersMC supplies only EMI's dedicated compile API artifact; every full EMI client/data runtime is resolved by exact Modrinth version ID. It uploads jar + logs/reports artifacts.
- Optional client boot/resource smoke lives in `.github/workflows/client-smoke.yml` and is `workflow_dispatch` only; it resolves the latest EMI accepted by `emi_version_range`, maps that API version to one exact NeoForge 1.21.1 Modrinth runtime, and is not GUI layout approval.

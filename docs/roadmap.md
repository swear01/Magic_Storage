...
- RS2 設計慣例採用(見 `docs/rs2-design-gap.md` + `docs/superpowers/plans/2026-06-21-...`):fuzzy 合成配對(A5b)、view 設定 server 同步(A5a/#7)、選取物品身分化(A4/#5)、合成 preview(A7)、Action/Actor storage contract(A2)、StorageListener 變更事件 foundation(A3)、捲軸/版面修正(#8)。P4 增量網路成長(A1:放置增量 + 破壞 full rebuild 後盾)。剩 P3 增量 grid(低價值,未來若改整列表 client grid 再做)。SelfTest 104 + GameTest 81 全綠。
...
- RS2 parity P0/P1/P2 pass:Core storage ops 新增 `Action` + `Actor` contract 與 `StorageListener` delta events;bus cached-core 會在斷網後失效;conflicted multi-core 網路不接受也不抽出;local terminal `stillValid` 以開啟的 terminal/access block 距離判斷、remote terminal 可遠端保持有效;crafting terminal recipe count/type/preview 由 server sync,client screen 不再自行掃 `RecipeManager` 或讀 core storage;craftable-only filter server 端套用;EMI 走 `StorageTerminalMenu.DISPLAY_SLOTS` 並只在 output 已在 grid 時 canCraft;duplicate ingredients 先彙總需求量再 preview/craft;crafting terminal 開啟時立即套用預設 compact grid。SelfTest 104 + GameTest 81 全綠。

...
| **GUI session / 需要目視時** | `python3 scripts/run_prism_gui_session.py --scenario terminal-left-rail`(會重建 wrapper + 固定 world + 本次 log polling + checklist) |
| Prism dev + Computer Use GUI 驗證 | 只在 scenario 顯示 `manual_gui_required=true` 或當次改動觸及可見 GUI/Patchouli 時進行;見下方「Prism dev / Computer Use」 |

GitHub public repo:https://github.com/swear01/Magic_Storage 。release tag 範例:`git tag v0.1.3 && git push origin main v0.1.3`。CI/CD 會保存 `build/ci-logs/**`、`run/logs/**`、`run/crash-reports/**`、`build/reports/**` 方便遠端失敗排查。`client-smoke.yml` 是 workflow_dispatch-only 的 client boot/resource smoke。GUI/Patchouli/視覺變更仍必須跑下方 Prism dev / Computer Use session runner;CI/GameTest/client-smoke 都不能替代目視 GUI 驗證。

手動測 GUI 優先用 session runner:`python3 scripts/run_prism_gui_session.py --scenario terminal-left-rail`。runner 會先呼叫 `scripts/setup_prism_computer_use_wrapper.py` 重建 `/tmp` Computer Use app/wrapper,再呼叫 `prepare_prism_gui_world.py` 重建 `MagicStorageGuiTest`、安裝無 command block datapack rig、啟用 cheats、把 use 鍵改成 `u` 並固定 1280×720 windowed 起跑,最後只讀本次 log offset 並輸出 `build/gui-runs/<timestamp>-<scenario>/checklist.md`。GUI 外觀無法用 GameTest/client-smoke 驗證;只有當 scenario `manual_gui_required=true` 或當次改動觸及可見 GUI/Patchouli 時才做 Computer Use 目視/截圖。
...
- Prism dev instance 設定:`~/Library/Application Support/PrismLauncher/instances/dev/instance.cfg` 需有 `OverrideCommands=true` 與 `WrapperCommand=/tmp/magic_storage_minecraft_cu_wrapper.sh`;不要手改,用 `python3 scripts/setup_prism_computer_use_wrapper.py` 或 `run_prism_gui_session.py` 重建/修補。
...
- 2026-07-11 已實測 `python3 scripts/run_prism_gui_session.py --scenario boot-smoke`:runner 開啟 Prism dev `MagicStorageGuiTest`,只讀本次 log offset,看到 `SelfTest: 104 passed`、Patchouli `preloaded 17 jsons`、`MS_GUI_TEST_READY`,且 current-run forbidden pattern scanner 未命中 `advanced_container_set_data` / `ERROR` / `FATAL` / `Caused by`。這是非目視 smoke;沒有取代下方全螢幕 GUI gate。

固定 GUI session 流程(先 windowed 直接進世界,再強制通過「全螢幕 gate」):

```bash
# 非視覺 smoke:只驗本次 client boot / SelfTest / ready log,不需要 Computer Use 目視
python3 scripts/run_prism_gui_session.py --scenario boot-smoke

# 終端/Patchouli/可見 GUI 變更:runner 產生 checklist,之後才用 Computer Use 目視
python3 scripts/run_prism_gui_session.py --scenario terminal-left-rail
```

`run_prism_gui_session.py` 會重建 Computer Use wrapper、重建固定 world、啟動 Prism `-w MagicStorageGuiTest`、只從本次 log offset polling `SelfTest:` + `MS_GUI_TEST_READY`,並把 `manifest.json`、`session.json`、`log-excerpt.log`、`checklist.md` 寫到 `build/gui-runs/<timestamp>-<scenario>/`。預設 `--timeout 300`,因為 Prism 偶爾會先做 Microsoft/Xbox/Minecraft profile refresh 而花超過 60 秒;不要改用固定短 sleep。若只要預產 artifacts 不啟動 Prism,可加 `--no-launch`。

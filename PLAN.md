...
- **Stations & Axe Energy** 有九個 descriptors：Furnace / Blast Furnace / Smoker / Campfire / Brewing Stand 是可堆疊 PROCESS，每個 block 每 tick 增加對應池 1；Crafting Table / Stonecutter / Smithing Table 是最多各一個、可取回且只解鎖 recipe 的 INSTANT；axe 是 CONSUMABLE input，不會留在 persistent slot。Core server-side 持久化前八格、finite `axeEnergy` 與 explicit `infiniteAxeEnergy`，都不占一般種類容量。Finite value = remaining durability × (vanilla Unbreaking level + 1) × count；Mending/其他 enchantment 不加值，overflow 必須拒絕且不吃物，Unbreakable 設為永不遞減的 ∞。Legacy slot 8 只在成功 conversion 後清除；失敗時由同一 Axe Energy 格顯示並允許取回。舊存檔若有堆疊的 INSTANT station，載入時保留一個安裝並把可表示的 extras 移回 Core 一般儲存，不能刪除。
...
- Fuel 頁由 `MachineEnergyTable.entries()` 與 ordered fuel-target descriptors 產生 tiles；**Stations & Axe Energy** 使用兩個 flow rows，前八格是 persistent process/instant slots，第九格通常是只轉換、不保存物品的 transient Axe Energy input並顯示 server-synced finite/∞ reserve；若 legacy conversion 失敗，該格先代理 Core slot 8 的原物供玩家取回，清空後才恢復 transient input。Wide machine/reserve panels 鋪滿 inner width，右下 `fuelControlPanel` 與玩家 inventory 同高並容納 target selector/Fuel input。每一頁的可見 descriptors 會平均鋪滿 panel，超過 capacity 時各自滾動分頁。左 rail 在 Fuel 頁只保留 Storage/Craftable/Fuel；單一 shared `TerminalCycleButton` 顯示目前 Auto/Fuel/Brew/Bottle target，left-click/scroll-down 往下一個、right-click/scroll-up 往上一個，server 只接 Auto/exact target IDs。旁邊的 list button 會從同一 ordered server-approved descriptors 建立代表物+名稱+selected state 的 bounded-scroll popup；選取、Escape、外部點擊或離開 Fuel 頁都會關閉並清 focus，彈窗攔截底層 slot input/tooltip且加入 EMI exclusion，不把 targets 塞回 rail。每種 energy 以對應 machine/fuel item 作代表物，Fuel 頁顯示 server-synced stored/max type capacity；`furnace_fuel` 對玩家只顯示 **Fuel**，內部 ID 不遷移。目前尚未暴露第三方 runtime registration；未來 API 必須 server-owned 並同步固定 descriptor/menu parity。
...
Crafting menu 兩端固定 149 slots（81 display + 36 player + 1 Fuel input + 8 persistent process/instant slots + 1 transient Axe Energy input + 22 presentation metadata）；Storage base data 固定 11，Crafting/Fuel 再加 89，總計 100 data slots。末五個 Crafting data values 是 Axe Energy 的四個 unsigned-16 words與 infinite flag。22 個 hidden slots 依序同步 exact output、9 個 item ledger representatives、station、9 個 positioned inputs、tool 與 typed metadata carrier；primitive availability/energy requirements仍以拆成 unsigned 16-bit words 的 data slots傳輸。Client 只由這些欄位重建 immutable `RecipePresentation`，不掃 `RecipeManager` 或 Core。目前 shared terminal/EMI/station/Axe revision 見 `docs/superpowers/specs/2026-07-14-terminal-platform-emi-recipe-axe-design.md` 與同名 implementation plan。
...
Crafting Terminal 由左側 rail 的第一群組切換 **Storage / Craftable / Fuel** 三頁，並用明顯間隔和 item-page 排序、搜尋 controls 分開。Storage 只列已儲存物，Craftable 只列目前可合成且不可抽取的 synthetic outputs；兩者都以 server-owned `TerminalDisplayStack` exact `long` metadata 顯示目前 Core 庫存，Craftable 零庫存 icon 仍存在但不顯示假數量。Name/Quantity/Mod/ID 走同一 deterministic comparator，display marker 不得進入 `ItemKey` 或真實 output。Fuel rail 只顯示三個 page tabs，machine/reserve tiles 依每頁可見 descriptor counts 平均鋪滿 bounds 並處理 overflow。Fuel target 由 Energy Reserves header 的 single current-value shared cycle control或旁邊的 bounded-scroll list popup 選擇，沒有語意混雜的 previous/next action buttons；popup 顯示 ordered targets 的代表物、名稱與 selected state，且完整處理 outside/Escape/focus/EMI exclusion。Fuel/Brew/Bottle reserve 分別用 Coal/Blaze Rod/Glass Bottle，Fuel 頁顯示 stored/max types；machine/reserve tooltip 只在實際 station slot/代表 icon 上觸發，不把 flow-cell padding 當 hover target。不同 component 的物品永遠維持完整 `ItemKey` 身分。`TerminalProfile` 組合 Storage reduced/Crafting full capabilities，所有 screens 只走 `TerminalLayout.forProfile(...)`；`StorageTerminalScreen` shared shell 是 common controls 唯一 owner，Crafting 不得 hide-and-recreate。Rail controls 全部是 18×18 hitbox/16×16 item-or-atlas icons，cycle input 一律 left-next/right-previous/wheel-down-next/wheel-up-previous。Grid amount 採 compact units，畫面只計算一個由最寬允許字串與 16px bound 得出的 fixed scale，每個值都用同一 scale 留在自己的 cell 內 right-align。所有動態 slots 委派 constructor 時保存的 original semantic slot，repeated `init()` 不會堆 wrapper。Fuel 常駐畫面不顯示 rate 公式，精確資料由 hover tooltip 發現，status text 無 shadow。合成列為 live-disabled `×1 / ×8 / ×64 / Max`，Max 由 server 重新計算當下合法最大值且不受 9,999 preview sync cap 截斷。

Block/item 材質固定為原版 16×16，採同一 dark deepslate/blackstone 機殼與 cyan/amethyst palette；Core rune、Storage item grid、Crafting 3×3 mark、T1–T6 六段容量條、藍色 inward Import、橘色 outward Export 與 Remote display 都有固定語意。抽象 rail controls 集中在 16px-cell `terminal_controls.png`，Name/Quantity/Mod/ID/Search/Tag/Mod/Previous/Next 不再使用尺寸不一的 Unicode。Replicate 候選、固定 seed/reference、selection manifest、清理腳本與 contact sheets 只留在 `art/texture-generation/20260714-terminal-family/`，不進 runtime jar。
...
| 左側 Fuel 頁按鈕 | 切到 Fuel 頁；rail 只保留三頁籤，再用 Energy Reserves header 的 Fuel Target cycle selector或 list popup 選 Auto/Fuel/Brew/Bottle、安裝機器、放入或 Shift+燃料，並以 panel paging 查看目前所有 energy descriptors |
...
| 語言 | Java 21 |
| 配方瀏覽 | **EMI**（release 為 client-only required dependency，接受 `[1.1.24,2)`；compile/dev runtime 用最低基準 `dev.emi:emi-neoforge:1.1.24+1.21.1`，CI 另編譯最新相容 1.21.1 版；dedicated server 不要求 EMI。Exact standard recipe 上方 diagram 只走公開 `EmiRecipe`/`Widget`/`WidgetHolder`，internal synthetic recipe 走 native renderer；handler 只接受 terminal 支援的 exact backing recipe，送 `{recipeId,amount,destination}`；NONE 選取 preview，CURSOR/INVENTORY 執行單層 server-authoritative magic craft，不做遞迴 pattern tree） |
...
class StorageCoreBlockEntity extends BlockEntity {
    ...
    // 能量池
    Map<EnergyType, Long> energy;          // process/Fuel 能量池，無上限
    Container machines;                    // slots 0..7 persistent process/instant stations
    long axeEnergy;                        // finite reserve；checked addition
    boolean infiniteAxeEnergy;             // explicit ∞，不以 Long.MAX_VALUE 代替
...
| M5 | 能量池系統（EnergyType、Core 持久化 process/instant slots、按安裝數量累積、燃料轉換、finite/∞ Axe Energy、RecipeType 映射、NBT） |

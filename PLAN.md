...
| 語言 | Java 21 |
| 配方瀏覽 | **EMI**（release metadata 可選整合；compile 使用 `dev.emi:emi-neoforge:1.1.24+1.21.1:api`，dev runtime 載入 full EMI；exact standard recipe 的上方 diagram 只走公開 `EmiRecipe`/`Widget`/`WidgetHolder`，無 EMI 或 internal synthetic recipe 才走 native renderer；handler 只接受 terminal 支援的 exact backing recipe，送 `{recipeId,amount,destination}`；NONE 選取 preview，CURSOR/INVENTORY 執行單層 server-authoritative magic craft，不做遞迴 pattern tree） |
